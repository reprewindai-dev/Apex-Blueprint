...
export default function App() {
  ...
  const [evidenceSubTab, setEvidenceSubTab] = useState<"cryptographic" | "academic">("cryptographic");
                ...
                <div className="space-y-6 animate-fadeIn">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2 border-b border-[#222] pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-xl font-black text-white uppercase tracking-tight">Evidence, Grounding & Cryptographic Provenance</h3>
                    </div>
                    <span className="px-2 py-0.5 bg-[#00F0FF]/10 text-[#00F0FF] border border-[#00F0FF]/20 text-[9px] font-mono uppercase font-black">
                      Sovereign Audit Level
                    </span>
                  </div>

                  {/* SUB-TABS SELECTOR */}
                  <div className="flex flex-wrap border-b border-[#222] pb-1 mb-4 gap-2">
                    <button
                      type="button"
                      onClick={() => setEvidenceSubTab("cryptographic")}
                      className={`px-3 py-1.5 text-[10px] font-mono uppercase tracking-widest transition-all rounded-none border ${
                        evidenceSubTab === "cryptographic"
                          ? "bg-[#00F0FF] text-black font-black border-[#00F0FF]"
                          : "text-[#666] hover:text-white border-transparent"
                      }`}
                    >
                      / Cryptographic Lineage (Poltergeist)
                    </button>
                    <button
                      type="button"
                      onClick={() => setEvidenceSubTab("academic")}
                      className={`px-3 py-1.5 text-[10px] font-mono uppercase tracking-widest transition-all rounded-none border ${
                        evidenceSubTab === "academic"
                          ? "bg-[#00F0FF] text-black font-black border-[#00F0FF]"
                          : "text-[#666] hover:text-white border-transparent"
                      }`}
                    >
                      / Academic Grounding & SSRN
                    </button>
                  </div>

                  {evidenceSubTab === "cryptographic" ? (
                    <AttestationManager blueprintHash={result?.hash} blueprintTitle={result?.title} />
                  ) : (
                    <>
