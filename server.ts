...
// ==========================================
// DYNAMIC ATTESTATION GENERATION ENDPOINT
// ==========================================

let triggerSequence = 14821;

app.post("/api/attestation/generate", (req, res) => {
  try {
    const workspaceRoot = process.cwd();
    
    // 1. Compute Source Tree Hash
    let sourceContent = "";
    const sourceFiles = [
      "src/App.tsx",
      "src/types.ts",
    ...
    if (!sourceContent) {
      sourceContent = "fallback_workspace_source_tree_context";
    }
    const sourceTreeHash = "sha256:" + crypto.createHash("sha256").update(sourceContent).digest("hex");

    // 2. Compute Dependency Lock Hash
    ...
    depFiles.forEach(file => {
      ...
      if (fs.existsSync(fullPath)) {
        try {
          dependencyContent += fs.readFileSync(fullPath, "utf-8");
        } catch (e) {
          // ignore
        }
      }
    });
    const dependencyLockHash = "sha256:" + crypto.createHash("sha256").update(dependencyContent || "fallback_dependency_lock_context").digest("hex");

    // 3. Compute Build Recipe Hash
    let recipeContent = "";
    const recipeFiles = ["vite.config.ts", "tsconfig.json"];
    recipeFiles.forEach(file => {
      const fullPath = path.join(workspaceRoot, file);
      if (fs.existsSync(fullPath)) {
        try {
          recipeContent += fs.readFileSync(fullPath, "utf-8");
        } catch (e) {
          // ignore
        }
      }
    });
    const buildRecipeHash = "sha256:" + crypto.createHash("sha256").update(recipeContent || "fallback_build_recipe_context").digest("hex");

    // 4. Compute Toolchain Digest
    const toolchainString = `${process.version}-${process.platform}-${process.arch}`;
    const toolchainDigest = "sha256:" + crypto.createHash("sha256").update(toolchainString).digest("hex");

    // 5. Compute Artifact Hash
    let artifactContent = sourceTreeHash + dependencyLockHash + buildRecipeHash;
    ...
    const manifestPayload = {
      ...
      build_recipe_hash: buildRecipeHash,
    ...
    const signatureInput = JSON.stringify(manifestPayload);
    const signature = "0x" + crypto.createHmac("sha256", "veklom-secret-signing-key").update(signatureInput).digest("hex");
    ...
    return res.status(500).json({ error: err.message || "Failed to generate attestation hashes." });
  }
});
